...
class Inputinap extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        is_login();
        $this->load->model('Tbl_input_inap_model');
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'inputinap/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'inputinap/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'inputinap/index.html';
            $config['first_url'] = base_url() . 'inputinap/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Tbl_input_inap_model->total_rows($q);
        $inputinap = $this->Tbl_input_inap_model->get_limit_data($config['per_page'], $start, $q);
        ...
        $data = array(
            'inputinap_data' => $inputinap,
            'q' => $q,
            'pagination' => $this->pagination->create_links(),
            'total_rows' => $config['total_rows'],
            'start' => $start,
        );
        $this->template->load('template','inputinap/tbl_pemeriksaan_rawat_inap_list', $data);
    }

    public function read($id) 
    {
        $row = $this->Tbl_input_inap_model->get_by_id($id);
        if ($row) {
            $data = array(
		...
		'keluhan' => $row->keluhan,
            ...
            $this->template->load('template','inputinap/tbl_pemeriksaan_rawat_inap_read', $data);
        } else {
            $this->session->set_flashdata('message', 'Record Not Found');
            redirect(site_url('inputinap'));
        }
    }
    ...
    public function create() 
    {
        $data = array(
            'button' => 'Create',
            'action' => site_url('inputinap/create_action'),
        ...
        $this->template->load('template','inputinap/tbl_pemeriksaan_rawat_inap_form', $data);
    }
    ...
    public function create_action() 
    {
		...
		'keluhan' => $this->input->post('keluhan',TRUE),
            ...
            $this->Tbl_input_inap_model->insert($data);
    ...
    public function update($id) 
    {
        ...
        if ($row) {
            $data = array(
                'button' => 'Update',
                'action' => site_url('inputinap/update_action'),
    ...
    public function update_action() 
    {
		...
		'keluhan' => $this->input->post('keluhan',TRUE),
            ...
            $this->Tbl_input_inap_model->update($this->input->post('id_periksa', TRUE), $data);
    ...
    public function delete($id) 
    {
        ...
        if ($row) {
            $this->Tbl_input_inap_model->delete($id);
	...
	foreach ($this->Tbl_input_inap_model->get_all() as $data) {
            $kolombody = 0;
    ...
    public function word()
    {
        ...
        $data = array(
            'tbl_pemeriksaan_rawat_inap_data' => $this->Tbl_input_inap_model->get_all(),
            'start' => 0
        );
        
        $this->load->view('inputinap/tbl_pemeriksaan_rawat_inap_doc',$data);
    }

}

/* End of file Inputinap.php */
/* Location: ./application/controllers/Inputinap.php */
