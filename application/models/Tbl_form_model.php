<?php

if (!defined('BASEPATH'))
    exit('No direct script access allowed');

class Tbl_form_model extends CI_Model
{

    public $table = 'tbl_pendaftaran';
    public $id = 'no_rawat';
    public $order = 'DESC';

    function __construct()
    {
        parent::__construct();
    }

    // get all
    function get_all()
    {
        $this->db->order_by($this->id, $this->order);
        return $this->db->get($this->table)->result();
    }

    // get data by id
    function get_by_id($id)
    {
        $this->db->where($this->id, $id);
        return $this->db->get($this->table)->row();
    }
    
    // get total rows
    function total_rows($q = NULL) {
        $this->db->like('no_rawat', $q);
	$this->db->or_like('no_registrasi', $q);
	$this->db->or_like('no_rekamedis', $q);
	$this->db->or_like('cara_masuk', $q);
	$this->db->or_like('tanggal_daftar', $q);
	$this->db->or_like('kode_dokter_penanggung_jawab', $q);
	$this->db->or_like('id_poliklinik', $q);
	$this->db->or_like('nama_penanggung_jawab', $q);
	$this->db->or_like('hubungan_dengan_penanggung_jawab', $q);
	$this->db->or_like('alamat_penanggung_jawab', $q);
	$this->db->or_like('id_jenis_bayar', $q);
	//$this->db->or_like('asal_rujukan', $q);
	$this->db->or_like('no_hp_penanggung_jawab', $q);
	$this->db->from($this->table);
        return $this->db->count_all_results();
    }

    // get data with limit and search
    function get_limit_data($limit, $start = 0, $q = NULL,$cara_masuk) {
        $this->db->where('tbl_pendaftaran.cara_masuk',$cara_masuk);
       // $this->db->where('tbl_pendaftaran.tanggal_daftar',date('Y-m-d'));
        $this->db->order_by('tbl_pendaftaran.no_rawat','DESC');
        /*
        $this->db->like('no_rawat', $q);
    $this->db->or_like('tbl_pasien.nama_pasien', $q);
	$this->db->or_like('tbl_pendaftaran.no_registrasi', $q);
	$this->db->or_like('tbl_pendaftaran.no_rekamedis', $q);
	$this->db->or_like('tbl_pendaftaran.cara_masuk', $q);
	$this->db->or_like('tbl_pendaftaran.tanggal_daftar', $q);
	$this->db->or_like('tbl_pendaftaran.kode_dokter_penanggung_jawab', $q);
	$this->db->or_like('tbl_pendaftaran.id_poliklinik', $q);
	$this->db->or_like('tbl_pendaftaran.nama_penanggung_jawab', $q);
	$this->db->or_like('tbl_pendaftaran.hubungan_dengan_penanggung_jawab', $q);
	$this->db->or_like('tbl_pendaftaran.alamat_penanggung_jawab', $q);
	$this->db->or_like('tbl_pendaftaran.id_jenis_bayar', $q);
	$this->db->or_like('tbl_pendaftaran.asal_rujukan', $q);
	$this->db->or_like('tbl_pendaftaran.no_hp_penanggung_jawab', $q);
    */

    //UNTUK MENJOINKAN TABEL
    $this->db->join('tbl_pasien','tbl_pasien.no_rekamedis=tbl_pendaftaran.no_rekamedis');
    $this->db->join('tbl_dokter','tbl_dokter.kode_dokter=tbl_pendaftaran.kode_dokter_penanggung_jawab');
    $this->db->join('tbl_poliklinik','tbl_poliklinik.id_poliklinik=tbl_pendaftaran.id_poliklinik');
    $this->db->join('tbl_jenis_bayar','tbl_jenis_bayar.id_jenis_bayar=tbl_pendaftaran.id_jenis_bayar');


	$this->db->limit($limit, $start);
        return $this->db->get($this->table)->result();
    }

    function get_data_pendaftaran($cara_masuk) {
        $this->db->where('tbl_pendaftaran.cara_masuk',$cara_masuk);
       // $this->db->where('tbl_pendaftaran.tanggal_daftar',date('Y-m-d'));
        $this->db->order_by('tbl_pendaftaran.no_rawat','DESC');

        //UNTUK MENJOINKAN TABEL
        $this->db->join('tbl_pasien','tbl_pasien.no_rekamedis=tbl_pendaftaran.no_rekamedis');
        $this->db->join('tbl_dokter','tbl_dokter.kode_dokter=tbl_pendaftaran.kode_dokter_penanggung_jawab');
        $this->db->join('tbl_poliklinik','tbl_poliklinik.id_poliklinik=tbl_pendaftaran.id_poliklinik');
        $this->db->join('tbl_jenis_bayar','tbl_jenis_bayar.id_jenis_bayar=tbl_pendaftaran.id_jenis_bayar');

        return $this->db->get($this->table)->result();
    }

    // insert data
    function insert($data)
    {
        $this->db->insert($this->table, $data);
    }

    // update data
    function update($id, $data)
    {
        $this->db->where($this->id, $id);
        $this->db->update($this->table, $data);
    }

    // delete data
    function delete($id)
    {
        $this->db->where($this->id, $id);
        $this->db->delete($this->table);
    }


    Function carikode($id){
    return $this->db->query("SELECT * FROM tbl_ruang_rawat_inap WHERE id_ruang_rawat_inap = '$id'")->row();
        }
}

/* End of file Tbl_form_model.php */
/* Location: ./application/models/Tbl_form_model.php */
/* Please DO NOT modify this information : */
/* Generated by Harviacode Codeigniter CRUD Generator 2021-03-18 15:36:35 */
/* http://harviacode.com */